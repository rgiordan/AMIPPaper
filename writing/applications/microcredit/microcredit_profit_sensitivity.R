...
load(file.path(output_dir, "microcredit_profit_data.Rdata"))
...
for(i in 1:K) {
  ...
  results_list[[i]] <- list(rerun_df=rerun_df, preds_df=preds_df, base_df=base_df, reg_fit=reg_fit)
}
...
save(results_df, base_df, file=file.path(output_dir, "microcredit_results.Rdata"))


save(results_df, base_df, signals, reruns, preds, 
     file=file.path(output_dir, "microcredit_detailed_results.Rdata"))
