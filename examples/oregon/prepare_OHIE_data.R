...
# ITT and LATE estimates from table 5 of the paper replicate perfectly and I run
...
oregon_dir <- file.path(git_repo_loc, "examples/oregon")
...
GetColumnLabels <- function(df) {
  ...
  for (col in colnames(df)) {
    col_labels <- bind_rows(
      col_labels,
      tibble(col=col, label=attr(df[[col]], "label"), type=typeof(df[[col]]))
    )
  }
  return(col_labels)
}
...
## Loading DTAs
raw_dta_desc <- read_dta(file.path(oregon_dir, "Data/OHI/OHIE_Public_Use_Files/OHIE_Data/oregonhie_descriptive_vars.dta"))
raw_dta_surveys <- c(file.path(oregon_dir, "Data/OHI/OHIE_Public_Use_Files/OHIE_Data/oregonhie_survey0m_vars.dta"),
                     file.path(oregon_dir, "Data/OHI/OHIE_Public_Use_Files/OHIE_Data/oregonhie_survey6m_vars.dta"),
                     file.path(oregon_dir, "Data/OHI/OHIE_Public_Use_Files/OHIE_Data/oregonhie_survey12m_vars.dta")) %>% 
  map(read_dta)
raw_dta_state <- read_dta(
  file.path(oregon_dir, "Data/OHI/OHIE_Public_Use_Files/OHIE_Data/oregonhie_stateprograms_vars.dta"))

print(GetColumnLabels(raw_dta_desc), n=Inf)
print(GetColumnLabels(raw_dta_surveys[[1]]), n=Inf)
print(GetColumnLabels(raw_dta_surveys[[2]]), n=Inf)
print(GetColumnLabels(raw_dta_surveys[[3]]), n=Inf)
print(GetColumnLabels(raw_dta_state), n=Inf)
...
# RJG: What are these three different surveys?  The paper says there were seven waves of surveys.
table(raw_dta_surveys[[1]]$in_survey_0m)
table(raw_dta_surveys[[2]]$in_survey_6m)
table(raw_dta_surveys[[3]]$in_survey_12m)
table(raw_dta_surveys[[1]]$in_survey_0m)
table(raw_dta_surveys[[2]]$in_survey_6m)
with(merge_df, table(in_survey_12m * in_survey_6m * in_survey_0m))
...
if (write_to_disk){
  write.csv(final_df, file = file.path(oregon_dir, "Data/OHI/OHIE_Public_Use_Files/OHIE_Data/data_for_analysis_R.csv"), row.names = FALSE)
}
rm(write_to_disk)
